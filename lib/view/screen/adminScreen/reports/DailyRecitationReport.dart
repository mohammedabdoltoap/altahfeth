...
class DailyRecitationReport extends StatelessWidget {
  final DailyRecitationReportController controller = Get.put(DailyRecitationReportController());

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("تقرير التسميع اليومي"),
      ...
      body: Column(
        children: [
          ...
          // Statistics
          Obx(() {
            if (controller.recitationList.isEmpty) return const SizedBox.shrink();
            
            final isSummary = controller.recitationList.first.containsKey('total_recitations');
            
            if (isSummary) {
              final totalRecitations = controller.recitationList.fold(0, (sum, s) => 
                sum + (int.tryParse(s['total_recitations'].toString()) ?? 0));
              final avgMark = controller.recitationList.fold(0.0, (sum, s) => 
                sum + (double.tryParse(s['avg_mark'].toString()) ?? 0.0)) / controller.recitationList.length;
              ...
              return Container(
                ...
                child: Row(
                  ...
                  children: [
                    _buildStatCard("عدد التسميعات", totalRecitations.toString(), Colors.purple),
                    _buildStatCard("متوسط الدرجات", avgMark.toStringAsFixed(1), Colors.orange),
                    _buildStatCard("عدد الطلاب", controller.recitationList.length.toString(), Colors.blue),
                  ],
                ),
              );
            } else {
              return Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    _buildStatCard("التسميعات", controller.recitationList.length.toString(), Colors.purple),
                  ],
                ),
              );
            }
          ...
          Expanded(
            child: Obx(() {
              ...
              if (controller.recitationList.isEmpty) {
                return const Center(
                  child: Text("لا توجد بيانات تسميع"),
                );
              }
            ...
            child: Column(
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    _buildStatItem('التسميعات', totalRecitations.toString(), Colors.purple),
...
class DailyRecitationReportController extends GetxController {
  var selectedDate = Rxn<DateTime>();
  var startDate = Rxn<DateTime>();
  var endDate = Rxn<DateTime>();
  var selectedCircle = Rxn<String>();
  var recitationList = <Map<String, dynamic>>[].obs;
